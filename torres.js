...
//Hcaemos true cuadro1 para que sea el contenedor inicial
...
function click1(){
	//es un interruptor, cambia entre true y false
...
//Esta funcion rellenara con espacios en blanco los cuadrados vacios
...
function Cuadro(cajaInicial){
	//Damos propiedades a los 3 contenedores main
	this.caja = crearDiv();
	...
	for (var i = 0; i < this.contenido.length; i++) {
		//esto añadirá las cajas a los contenedores-
	...
	this.tieneFichas = function(){
		...
		for(var  i = 0; i < this.contenido.length; i++){
			//instanceof se usa para decir si es del mismo tipo algo
	...
	};//se usa ; por ser una funcion dentro de otra
	...
	this.quitarFichaSuperior = function(){
		for(var i = 0; i < this.contenido.length; i ++){
			if (!(this.contenido[i] instanceof Relleno)){
				...
				//suplantamos ficha que movimos por una en blanco
	...
	this.reDibujarCaja = function(){
		//esto nos dira si la caja tiene dentro algo
...
function victoria(){
	var textoTitulo = document.createTextNode("¡¡Victoria!!");
	var textoSubtitulo= document.createTextNode("Movimientos utilizados: "+movimientos);
	var textoConsejo = document.createTextNode("Recarga para vovler a jugar");
...
function reiniciarOrigenDestino(){
	...
	//Nos aseguramos de que se deselecionen los cuadros
...
function iniciar(){
	//Nos permite recuperar todos los elementos con el mismo nombre de etiqueta
	cuerpo = document.getElementsByTagName("body")[0];
	//se da la posicion 0 en el arreglo pues ahi esta el body
	cuerpo.style.textAlign = "center";

	cuerpo.appendChild(cuadro1.caja);
	cuerpo.appendChild(cuadro2.caja); //Aqui mostramos las cajas en el body
